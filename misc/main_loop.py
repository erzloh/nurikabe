...
while dephth < 500:
    ...
    if doChecks(table):
        print("Nurikabe complete1"); ns.printTableOld(table); exit("success")
#--------------------------------------------------------------------------------------
    #not bad state
        ...
        if theChosenOne == None:
            ...
            if len(potentialNewTiles) > 0:
                ...
                if returningFromSameIsland:
                    ...
                    print("Not operating on the same island as before, appending table to stateHistory, it lookd like this:")
...
#----------------------------------------------------------------------------------------------
    #returning from bad state
        ...
        if len(stateHistory) > 0:
            ...
            # exit("Trying to get object from empty stateHistory")
            currentState = copy.deepcopy(ns.state(originalTable))
        print("currentState is now the last element from the state history, it looks like this:")
...
#----------------------------------------------
    ...
    if dephth == 14:
        print("islandHistory;",islandHistory)
