...
# partList = []
def islandParts(x, y, table, counter, partList=[], tempTable=[], revertTable=[],mode="complete", returning=False, firstrun=True):
    # print("iteration of islandParts on", x, y)
    if firstrun: #ICI ICI ICI ICI ICI ICI ICI ICI ICI ICI ICI ICI ICI ICI ICI ICI ICI ICI ICI ICI ICI ICI ICI ICI ICI ICI
        partList.clear()
        tempTable.clear()
        revertTable.clear()
    x_len = len(table)
    ...
    if x > 0 and table[x - 1][y] == "W" and (x - 1, y) not in tempTable:  # print("left")
        return islandParts(x - 1, y, table, counter, partList,tempTable, revertTable, mode, returning=False, firstrun=False)
    elif y > 0 and table[x][y - 1] == "W" and (x, y - 1) not in tempTable:  # print("up")
        return islandParts(x, y - 1, table, counter, partList,tempTable, revertTable, mode, returning=False, firstrun=False)
    elif (x < x_len - 1) and table[x + 1][y] == "W" and (
    x + 1, y) not in tempTable:  # I wonder if the < x_len-1 works in all cases ###print("right")
        return islandParts(x + 1, y, table, counter, partList,tempTable, revertTable, mode, returning=False, firstrun=False)
